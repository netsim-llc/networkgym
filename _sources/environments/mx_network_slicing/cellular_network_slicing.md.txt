...
```json
{
  ...
  "env_config":{
    ...
    "per_slice_config":{//the length of the following parameter, e.g., len("num_users"), must equal the size of "num_slices"!
      "num_users":[2,3],
      "dedicated_rbg":[0,0], //resource block group (rbg) size maybe 1, 2, 3 or 4, it depends on the resource block num, see table 7.1.6.1-1 of 36.213
      "prioritized_rbg":[13,12],
      "shared_rbg":[25,25],
      "packet_size_bytes":[1400, 1200], //The transport layer packet size. If the packet size is more than MTU (typically 1400 bytes), it may be fragmented by the network layer.
      "transport_protocol":["udp","TcpBbr"], //"udp" or "tcp". Select "tcp" will initiate "TcpBbr" algorithm by default, but you have the option to use other [supported protocols](https://www.nsnam.org/docs/models/html/tcp.html#congestion-control-algorithms) in ns3 by specify the tcp algorithm name, such a "TcpCubic".
      "udp_poisson_arrival":[false,false], // if "transport_protocol" is "udp", this para controls whether the data is generated using poisson process.
      "min_udp_rate_per_user_mbps":[2,2], // if "transport_protocol" is "udp", this para controls the min sending rate.
      "max_udp_rate_per_user_mbps":[2,2], // if "transport_protocol" is "udp", this para controls the max sending rate.
      "qos_loss_target":[0.01, 1], //max loss for qos flow. Qos rate is zero if loss is more than this target.
      "qos_delay_violation_target":[0.01, 0.01], //max delay violation for a qos flow. A packet violates delay if its owd is greater than qos_delay_requirement_ms. Qos rate is zero if delay violation is more than this target.
      "qos_delay_requirement_ms":[10, 1000000], //max delay for qos flow, a qos flow can tolerate qos_delay_violation_target(e.g., 1%) of packet violates this delay.
      "delay_test_1_thresh_ms":[100, 1000000], //only for delay violation test 1 measurement, this para does not impact flow qos.
      "delay_test_2_thresh_ms":[1000, 1000000], //only for delay violation test 2 measurement, this para does not impact flow qos.
      "downlink_mode":["auto", "auto"], //"auto", "split", or "steer". "auto" will config UDP and TCP ACK as steer and TCP data as split.
      "uplink_mode":["auto", "auto"] //"auto", "split", or "steer". "auto" will config UDP and TCP ACK as steer and TCP data as split.
    },
...
40              [1]             ul::network::rate    gma  6000           [{'slice': [0, 1], 'value': [0.0, 0.0]}]
41              [1]         ul::network::qos_rate    gma  6000           [{'slice': [0, 1], 'value': [0.0, 0.0]}]
42              [1]          dl::network::tx_rate    gma  6000       [{'slice': [0, 1], 'value': [3.998, 6.168]}]
43              [1]  ul::network::delay_violation    gma  6000           [{'slice': [0, 1], 'value': [0.0, 0.0]}]
44              [1]             dl::network::rate    gma  6000       [{'slice': [0, 1], 'value': [4.112, 6.048]}]
45              [1]         dl::network::qos_rate    gma  6000       [{'slice': [0, 1], 'value': [4.112, 6.048]}]
46              [1]          ul::network::tx_rate    gma  6000           [{'slice': [0, 1], 'value': [0.0, 0.0]}]
47              [1]  dl::network::delay_violation    gma  6000           [{'slice': [0, 1], 'value': [0.0, 0.0]}]
```
