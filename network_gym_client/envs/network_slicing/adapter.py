...
class Adapter(network_gym_client.adapter.Adapter):
    ...
    def __init__(self, config_json):
        ...
        self.size_per_feature = int(self.config_json['env_config']['num_slices'])
        self.num_features = 3
    ...
    def get_observation(self, df):
        ...
        dl_network_tx_rate = None
        dl_network_rate = None
        dl_network_qos_rate = None
        dl_network_delay_violation = None
        ...
        for index, row in df.iterrows():
            if row['source'] == 'lte':
                ...
                if row['name'] == 'dl::network::tx_rate':
                    dl_network_tx_rate = row
                elif row['name'] == 'dl::network::rate':
                    dl_network_rate = row
                elif row['name'] == 'dl::network::qos_rate':
                    dl_network_qos_rate = row
                elif row['name'] == 'dl::network::delay_violation':
                    dl_network_delay_violation = row

        self.wandb_log_buffer_append(self.slice_df_to_dict(dl_cell_rb_usage))
        self.wandb_log_buffer_append(self.slice_df_to_dict(dl_network_tx_rate))
        self.wandb_log_buffer_append(self.slice_df_to_dict(dl_network_rate))
        self.wandb_log_buffer_append(self.slice_df_to_dict(dl_network_qos_rate))
        self.wandb_log_buffer_append(self.slice_df_to_dict(dl_network_delay_violation))

        #warning, need to modify the following if use more than one base stations.
        observation = np.vstack([pd.json_normalize(dl_network_rate["value"])["value"].to_list(), pd.json_normalize(dl_cell_rb_usage["value"])["value"].to_list(), pd.json_normalize(dl_network_delay_violation["value"])["value"].to_list()])
